...
class WebFormPage():
    ...
    def select_date(self,mmddyyyy_string):
        ...
        #Generate today's date so I can select the switcher
        today_full_date_array = date.today().strftime("%m/%d/%Y").split('/')
        today_month = today_full_date_array[0]
        today_year = today_full_date_array[2]

        #Create the string to locate the switcher
        switcher_string = '{} {}'.format(calendar.month_name[int(today_month)],today_year)
        local_xpath_switcher = self.xpath_switccdher.format(switcher_string)
        date_picker_switch = self.browser.find_element(By.XPATH, local_xpath_switcher)

        #Now Switcher is selected and I can compare year, and if there is no match, I proceed to search
        time.sleep(3)
        switcher_text_content = date_picker_switch.get_attribute('textContent')
        if((target_date_year in switcher_text_content) and (calendar.month_name[int(target_month)] in switcher_text_content)):
